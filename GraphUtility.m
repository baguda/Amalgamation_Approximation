classdef GraphUtility
    methods(Static)
        ...
        %% Visualize Interaction Hamiltonian
        function visualizeHamiltonian(interactionHamiltonian, name)
            if isempty(interactionHamiltonian)
                disp('No interaction Hamiltonian to visualize.');
                return;
            end
            
            % Check if interactionHamiltonian is numeric or symbolic
            if isnumeric(interactionHamiltonian)
                % Plot the numeric Hamiltonian
                figure;
                plot(interactionHamiltonian);
                title([name, ' - Interaction Hamiltonian']);
                xlabel('x'); ylabel('Hamiltonian Value');
            elseif isa(interactionHamiltonian, 'sym')
                % Plot the symbolic Hamiltonian
                figure;
                fplot(interactionHamiltonian);
                title([name, ' - Symbolic Interaction Hamiltonian']);
                xlabel('x'); ylabel('Hamiltonian Value');
            else
                error('Unsupported interaction Hamiltonian data type.');
            end
        end
        
        %% Visualize Kernel Function
        function visualizeKernel(kernelFunction, name)
            if isempty(kernelFunction)
                disp('No kernel function to visualize.');
                return;
            end
            
            % Check if kernelFunction is numeric or symbolic
            if isnumeric(kernelFunction)
                % Plot the numeric kernel
                figure;
                plot(kernelFunction);
                title([name, ' - Kernel Function']);
                xlabel('x'); ylabel('Kernel Value');
            elseif isa(kernelFunction, 'sym')
                % Plot the symbolic kernel
                figure;
                fplot(kernelFunction);
                title([name, ' - Symbolic Kernel Function']);
                xlabel('x'); ylabel('Kernel Value');
            else
                error('Unsupported kernel function data type.');
            end
        end
    end
